...
results_json = os.environ.get('ALL_REQUIRED_CHECKS_COMPLETE_RESULTS', '{}') or sys.exit(1)

try:
    results = json.loads(results_json)
except json.JSONDecodeError:
    print(f"Error: Unable to parse ALL_REQUIRED_CHECKS_COMPLETE_RESULTS as JSON")
    exit(1)
...
for job_name, job_data in results.items():
    if not isinstance(job_data, dict):
        print(f"Unexpected shape at key {job_name}, expected a dict")
        sys.exit(2)
